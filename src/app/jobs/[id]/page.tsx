...
function JobDetailContent({ jobIdPromise }: { jobIdPromise: Promise<{ id: string }> }) {
  ...
  if (!resolved) {
    return <div className="jobs-loading-wrapper"><i className="pi pi-spin pi-spinner jobs-loading-icon" /></div>;
  }

  if (isLoading) return <div className="jobs-loading-wrapper"><i className="pi pi-spin pi-spinner jobs-loading-icon" /></div>;
    ...
    <div className="space-y-6">
      ...
      {data.error && (
        <div className="jd-error-box">
          <span className="jd-error-label">Error: </span>{data.error}
        </div>
      )}
        ...
        <div className="jd-retry-wrapper">
          ...
          >
            {retryMutation.isPending ? (
              <><i className="pi pi-spin pi-spinner jd-retry-spinner" />Retrying…</>
            ) : (
              <><i className="pi pi-refresh jd-refresh-icon" />Retry {failedCount} failed song{failedCount > 1 ? 's' : ''}</>
            )}
          </button>
        </div>
      )}
            ...
            <thead>
              <tr className="jd-table-header">
                {["#", "Title", "Artist", "Status", "Error"].map((h) => (
                  <th key={h} className="jd-table-th">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {data.songs.map((song) => (
                <tr key={song.id} className="jd-table-row">
                  <td className="jd-table-td jd-table-td--track">{song.track_number ?? "\u2014"}</td>
